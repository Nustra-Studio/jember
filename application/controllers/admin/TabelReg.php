...
class TabelReg extends CI_Controller {

	public function __construct(){
		parent::__construct();
		$this->load->model('TabelregModel','Model');
	...
	{
		$datacontent['url']='admin/tabelreg';
		$datacontent['title']='Halaman Tabel';
		$datacontent['datatable']=$this->Model->get();
		$data['content']=$this->load->view('admin/tabelreg/tableView',$datacontent,TRUE);
		$data['js']=$this->load->view('admin/tabelreg/js/tableJs',$datacontent,TRUE);
	...
	public function form($parameter='',$id='')
	{
		...
		$data['content']=$this->load->view('admin/tabelreg/formView',$datacontent,TRUE);
		$data['js']=$this->load->view('admin/tabelreg/js/formJs',$datacontent,TRUE);
		$data['title']=$datacontent['title'];
		$this->load->view('admin/layouts/html',$data);
	}
				...
				if($upload['info']==true){
					...
					foreach ($csv as $row) {
						$data[]=[
							...
							'id_kategori_hotspot'=>$id_kategori_hotspot[strtolower($row[8])],
							'keterangan'=>$row[4],
							'lokasi'=>$row[2],
							'lat'=>$row[5],
							'lng'=>$row[6],
					    ...
					    foreach (glob($newFolder.'/*.txt') as $file) {
							...
							foreach ($txt as $row) {
								$data[]=[
									...
									'id_kategori_hotspot'=>$id_kategori_hotspot[strtolower($row[6])],
									'keterangan'=>$row[2],
									'lokasi'=>$row[1],
									'lat'=>$row[3],
									'lng'=>$row[4],
	...
	public function datatable(){
		...
		$url = 'admin/hotspot';
		$kolom =['id_hotspot','norm','nik','tanggal','nama','tgl','kerja','kk','lokasi','nm_kecamatan','keterangan','lat','lng','nm_kategori_hotspot','nm_kategori_hotspot'];
